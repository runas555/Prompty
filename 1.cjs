...
// Синтаксически корректный код с правильной генерацией шаблонных строк для Next.js
              ...
              <div className="flex-1 flex flex-col overflow-hidden bg-slate-950 select-text font-mono text-xs">
                <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800 bg-indigo-950/20">
                  <span className="text-xs font-bold text-indigo-400 flex items-center gap-1.5 select-none">
                    ...
                    {t("studioCompareTitle")} v{selectedCompareVersion.version} ({t("studioCompareLeft")}) ↔ {t("studioCompareRight")}
                  </span>
                  <button
                    type="button"
                ...
                <div className="flex-1 flex overflow-hidden font-mono text-xs select-text">
                  <div className="flex-1 border-r border-slate-800 flex flex-col overflow-hidden">
                    <div className="bg-slate-900/80 px-3 py-1.5 text-[10px] text-slate-500 font-bold border-b border-slate-850 select-none">
                      {t("studioSnapshot").toUpperCase()} v{selectedCompareVersion.version}
                    </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              {agent && (
                <div className="flex border border-slate-800 bg-slate-950/40 rounded-xl overflow-hidden p-1 gap-1">
                  ...
                  >
                    {t("studioSnapshots")} ({versions.length})
                  </button>
                </div>
              )}
                        ...
                        <div key={ver.id} className="border border-slate-800 rounded-xl bg-slate-950/40 p-3 flex flex-col gap-2 text-xs">
                          <div className="flex items-center justify-between">
                            <span className="font-mono font-bold text-indigo-400">
                              {t("studioSnapshot")} v{ver.version} {idx === 0 && <span className="text-[9px] text-emerald-400 font-sans ml-1">({t("studioCurrent")})</span>}
                            </span>
                          ...
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setError(\`\${t("studioLoadedSnapshot")}\${ver.version}\`);
...
try {
  // Полная перезапись файла корректным кодом без лишних символов экранирования
  fs.writeFileSync(targetFile, cleanCode, 'utf8');
  console.log('[УСПЕШНО] Ошибка экранирования полностью устранена. Ползунок перевода активен и нажимается.');
} catch (err) {
  console.error('[ОШИБКА] Не удалось записать изменения в файл PostModal.tsx:', err.message);
}